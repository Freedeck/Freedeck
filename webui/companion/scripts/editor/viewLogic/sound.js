...
const true_selector_control = document.querySelector("#section-control-selector")
...
true_selector_control.onchange = (e) => {
  ...
  if(!featureFlags[i]) {
    universal.sendToast("This feature is not fully supported yet and may be broken, sorry! It will become available in a future release.", "Freedeck")
    true_selector_control.selectedIndex = 0;
    return;
  }
...
class Sound extends EditorViewLogic {
  constructor() {
    super("audio", "fd.sound", "fd.stopall", "fd.sb.pitch", "fd.sb.vol.out", "fd.sb.vol.mon");
...
document.querySelector("#upload-sound").onclick = () => {
  ...
  universal.listenForOnce("library_load", ()=>{setupLibraryFor("sound")});
  universal.listenForOnce("library_paint", () => {
    ...
    for (const uploadedIcon of document.querySelectorAll(".uploads-0 .upload")) {
      uploadedIcon.onclick = () => {
        ...
        loadData(ito.data);
        document.querySelector("#file.editor-data").value = uploadedIcon.dataset.name;
        document.querySelector("#path.editor-data").value = "/sounds/";
        document.querySelector("#audio-file").innerText = uploadedIcon.dataset.name;
