...
const style = new SidebarSection(
	translationKey("sidebars.left.soundboard.title"),
	"Soundboard",
);
...
style.children.push(
	new SidebarButton(
		translationKey("sidebars.left.soundboard.stop_all"),
		(e) => {
...
style.children.push(
	new SidebarSlider(
		translationKey("sidebars.left.soundboard.sliders.pitch"),
		"pitch",
		translationKey("sidebars.left.soundboard.sliders.pitch.unit"),
		"0.1",
		"2",
		"1",
		(e) => {
...
style.children.push(
	new SidebarSlider(
		translationKey("sidebars.left.soundboard.sliders.volume.output"),
		"v",
		translationKey("sidebars.left.soundboard.sliders.volume.unit"),
		"0",
		"100",
		"100",
		(e) => {
			universal.audioClient.setVolume(
				e.target.value / 100,
...
style.children.push(
	new SidebarSlider(
		translationKey("sidebars.left.soundboard.sliders.volume.monitor"),
		"mv",
		translationKey("sidebars.left.soundboard.sliders.volume.unit"),
		"0",
		"100",
...
const playbackModeSelect = new SidebarSelect(
	translationKey("sidebars.left.soundboard.mode.title"),
	"es-playback",
	(e) => {
...
universal.listenFor("loadHooks", () => {
	...
	if (universal.load("vol-0")) setValue("#v", universal.load("vol-0") * 100);
	if (universal.load("vol-1")) setValue("#mv", universal.load("vol-1") * 100);
});
