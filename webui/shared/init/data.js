...
export default async function dataHandler(universal, user) {
	...
	universal.CLU("Incoming Data Handler", "Listening for Identify event.");
	universal.ui = UI;
	universal.on("I", async (gzipped) => {
		...
		if (!document.querySelector("#keys")) {
			universal.CLU(
				...
				"Appending/Creating keys to body.",
			);
		...
		universal.CLU(
			"Incoming Data Handler",
			"Setting up plugins for Tile Editor.",
		);
		...
		if (user === "Companion") {
			...
			universal.CLU(
				"Incoming Data Handler",
				"Attempting to open NB socket / FreedeckApp",
			);
