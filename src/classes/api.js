...
class Plugin {
	...
	add(type, file) {
		...
		switch (type) {
			...
			case HookRef.types.socket:
				...
				console.log(
					"!!! This functionality has been built in to 'this.io' in your plugin!",
				);
				console.log(
					"!!! You need to request the intents: intents.IO, intents.SOCKET for the server & client.",
				);
				console.log(
					"!!! You need to request the intent: intents.CLIENTS for the clients.",
				);
				break;
	...
	addView(view, file) {
		...
		if (!fs.existsSync(path.resolve(viewBase, this.id)))
			fs.mkdirSync(path.resolve(viewBase, this.id), { recursive: true });
		this.internalAdd(
			HookRef.types.view,
			viewFolder,
			path.resolve(viewBase, this.id),
		);
	}
	...
	addDashImport(file) {
		...
		this.internalAdd(
			HookRef.types.dashModule,
			viewFolder,
			path.resolve(viewBase),
		);
	}
   ...
   */
	...
	register(
		...
		const existingType = this.types.filter((e) => {
			e.type === basic.type;
		});
		...
		return pluginManager.types().set(typeData.type, {
			instance: this,
			...basic,
